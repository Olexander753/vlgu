...
int main(int argc, char* argv[]) {
	...
	Engineer ass1(std::string("Синицын"), 16);
	Engineer ass2(std::string("Грушин"), 19);
	...
	//
	vlsuLibrary.LetLibItemOut(vlsuLibrary.GetLibItems()[2], &st1);
	vlsuLibrary.LetLibItemOut(vlsuLibrary.GetLibItems()[1], &ass1);
	regionLibrary.LetLibItemOut(regionLibrary.GetLibItems()[1], &st3);

	vlsuLibrary.LetLibItemOut(vlsuLibrary.GetLibItems()[0], &lec1);
	regionLibrary.LetLibItemOut(regionLibrary.GetLibItems()[0], &lec1);
	regionLibrary.LetLibItemOut(regionLibrary.GetLibItems()[1], &ass2);
...
void Show(Library& lib) {
	...
	for (int i = 0; i < items.size(); i++) {
		...
		Vocabulary* geo_map = dynamic_cast<Vocabulary*>(items[i]->GetItem());
		...
		if (book != NULL) {
			...
			if (geo_map != NULL) {
				std::cout << " язык " << geo_map->GetLanguage();
			}
		}
